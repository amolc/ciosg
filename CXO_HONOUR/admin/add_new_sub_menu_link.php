<?php
	...
	if(isset($_SESSION['admin'])){
		...
		$id = $_REQUEST['id'];
		$result = mysql_query("SELECT * FROM menu WHERE menu_id=$id");
...
$(document).ready(function(){
	/**
			* tour
			*@author:	developerdevday7@outlook.com
...
<?php include('include/admin_header/admin_header.php'); ?>
			...
			<ol class="breadcrumb bc-3">
				...
				<li class="">
					<strong><a href="admin_menu.php">Menu</a></strong>
				</li>
				<li class="">
					<strong><a href="admin_sub_menu.php?id=<?php echo $id; ?>"><?php echo $row['menu_name']; ?></a></strong>
				</li>
				<li class="active">
			...
			<div class="panel-body">
				...
				<?php 
					if($_POST['submit'] == 'submit')
					{
						...
						$parent_id = mysql_real_escape_string($_POST['id']);
							...
							$sql   = "insert into sub_menu(sub_name,sub_link,parent_id,sub_created_date) values ('$title','$link','$parent_id','$insert_date')";
							$query = mysql_query($sql) or dei(mysql_error());
							if($query)
							{
								
								echo'<script>window.location.replace("admin_sub_menu.php?id='.$parent_id.'&add=ok");</script>';
				...
				<form   method="post" role="form" id="form1"   class="form-horizontal validate" enctype="multipart/form-data" action='<?php echo $_SERVER['PHP_SELF'] ;?>'>
						...
						<div class="form-group">
							...
							<div class="col-sm-5">
								<input style="margin-left: 20px;" type="text" data-validate="required" data-message-required="This is  required field."  class="form-control" name="title" id="title" placeholder="Menu Title" required>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-5">
								<input style="margin-left: 20px;" type="text" data-validate="required" data-message-required="This is  required field."  class="form-control" name="link"  placeholder="URL" required>
							</div>
						</div>
						...
						<input style="margin-left: -188px;margin-top: 10px;" type="submit" name="submit" value="submit" class="btn btn-info ok" />
