		...
		<?php
			...
			/*
				...
				if ((($_FILES["file"]["type"] == "image/gif")|| ($_FILES["file"]["type"] == "image/jpeg")|| ($_FILES["file"]["type"] == "image/jpg")
						...
						|| ($_FILES["file"]["type"] == "image/png"))&& ($_FILES["file"]["size"] < 220000000000) && in_array($extension, $allowedExts))
				{
					...
					if(in_array($ext,$valid_formats))
					{
						if($size<(60000000))
						{
							...
							{	
								...
								$advisory_industry = mysql_real_escape_string($_POST['advisory_industry']);
								$advisory_years = mysql_real_escape_string($_POST['advisory_years']);
								$advisory_image1 = $_FILES["file"]["name"]; 
								$advisory_image = "timthumb.php?src=admin/advisory_panel_img/".$advisory_image1; 
								
								$advisory_description = addslashes($_POST['advisory_description']);

								$today_date = mktime(0,0,0,date("m"),date("d"),date("Y"));
								$current_date = date("m/d/Y", $today_date);
								
									$sql   = "insert into advisory_panel(advisory_name,advisory_desination,advisory_company,advisory_image,advisory_description,advisory_industry,advisory_years,advisory_insert_date) values('".$advisory_name."','".$advisory_desination."','".$advisory_company."','".$advisory_image."','".$advisory_description."','".$advisory_industry."','".$advisory_years."','".$current_date."')";
							
									$query = mysql_query($sql);
									if($query)
									{
										echo "<h1>Advisor Add Successful</h1>";
									}
									else 
									{
										echo "error";
							...
							echo "<h1>Image file size max 1 MB</h1>";					
					}
					else
						echo "<h1>Invalid file format..</h1>";	
							...
							// $current_date = date("m/d/Y", $today_date);
								...
								// $sql   = "insert into advisory_panel(advisory_name,advisory_desination,advisory_company,advisory_image,advisory_description,advisory_industry,advisory_insert_date) values('".$advisory_name."','".$advisory_desination."','".$advisory_company."','".$advisory_image."','".$advisory_description."','".$advisory_industry."','".$current_date."')";
						
								// $query = mysql_query($sql);
								// if($query)
								// {
									// echo "<h1>Logo Change successful</h1>";
								// }
								// else 
								// {
									// echo "error";
				...
				<form role="form" action="<?php $_SERVER["PHP_SELF"];?>" method="post"  enctype="multipart/form-data" class="form-horizontal form-groups-bordered">
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" class="form-control" id="field-1" name="advisory_name"  placeholder="" required>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" class="form-control" id="field-1" name="advisory_desination"  placeholder=" " required>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" class="form-control" id="field-1" name="advisory_company"  placeholder=" " required>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" class="form-control" id="field-1" name="advisory_industry"  placeholder=" " required>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" class="form-control" id="field-1" name="advisory_years"  placeholder=" " required>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
							...
							<div class="form-group" style="margin-left: 291px; width: 506px;">
<!--								<textarea style="width:423px;height:140px;" class="form-control wysihtml5 overview" name="advisory_description" id="tour_overview" required></textarea>-->
							
                                                                <?php
                                                        include("plugin/fckeditor/fckeditor.php") ;

                                                          $oFCKeditor = new FCKeditor('advisory_description') ;
