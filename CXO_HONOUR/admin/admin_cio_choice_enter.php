<?php
	...
	if(isset($_SESSION['admin'])){
		...
		$id = $_REQUEST['id'];
		$result = mysql_query("SELECT * FROM enter_page where enter_type='enter' ");
					...
					if($_POST['submit'] == 'submit')
					{
                        ...
                        $sql   = "Update enter_page SET
						   enter_description = '$enter_description', enter_heading = '$heading'
							where enter_type = 'enter'
							";
							$query = mysql_query($sql) or die (mysql_error());
							if($query)
							{
								echo'<script>window.location.replace("admin_cio_choice_enter.php?edit=ok");</script>';
