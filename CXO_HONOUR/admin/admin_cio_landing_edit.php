...
include('../sql_config/database/cio_db.php'); 
 $id = $_REQUEST['id'];
 $res = mysql_query("select * from cio_landing  where id = '$id'")or die (mysql_error());
	...
	<div class="col-md-12">
		...
		<?php
			...
			if($_POST['upload_button'] == "Submit"){
				...
				if (in_array($_FILES['file']['type'], $allowedImageTypes)&& ($_FILES["file"]["size"] < 120000))
				  {
				  if ($_FILES["file"]["error"] > 0)
					...
					if (file_exists("landing_images/cio/" . $_FILES["file"]["name"]))
					  {
					  	 $fileData = pathinfo(basename($_FILES["file"]["name"]));
					   	 $fileName = uniqid() . '.' .$fileData['extension'];
						 $target_path = 'landing_images/cio/'.$fileName;
					     move_uploaded_file($_FILES["file"]["tmp_name"],$target_path);
					     echo "Stored in: " . "landing_images/cio/" . $target_path;
					  }
					else
					  {
				       $fileData = pathinfo(basename($_FILES["file"]["name"]));
					  ...
					  echo "Stored in: " . "landing_images/cio/" .  $target_path;
						...
						/*
							...
							*@AUTHER:	developerdevday7@outlook.com
						...
						$result = mysql_query("SELECT id FROM cio_landing WHERE id = '$id'");
						...
						if ($num_rows > 0)
						{
							...
							$sql   = "UPDATE cio_landing SET title='".$title."',description='".$description."',insert_date='".$insert_date."',path='".$path."' WHERE id = '$id'";
						...
						else
						{
							...
							$sql   = "insert into cio_landing(title,description,insert_date,path) values ('$title','$description','$insert_date','$path')";       
