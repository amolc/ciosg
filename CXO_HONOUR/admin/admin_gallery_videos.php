...
<h2>Videos</h2>
<div style="float:right"><a href="admin_add_gallery_video.php?id=<?php echo $id; ?>"><button type="button" class="btn btn-success">Add New</button></a></div>
<br />
<br />

<?php if(isset($_REQUEST['del'])){ 
echo '<h2 style="color:red;">Video Deleted</h2>';
}
if(isset($_REQUEST['edit'])){ 
echo '<h2 style="color:green;">Video Updated</h2>';
}
if(isset($_REQUEST['add'])){ 
echo '<h2 style="color:green;">Video Saved</h2>';
}
?>
...
$result = mysql_query("SELECT
						*
						FROM
						gallery_videos where gallery_id = '$id' 
						");
		...
		{ 
			echo'
				<tr class="odd gradeX" id="'.$row['video_id'].'">				    
					<td>'.$row['video_title'].'</td>';
									
			echo '</td><td>'.$row['video_date'].'</td>
					<td><a href="admin_edit_gallery_video.php?id='.$row['video_id'].'&pid='.$id.'" class="btn btn-default btn-sm btn-icon icon-left edit_tour_list">
							<i class="entypo-pencil"></i>
							Edit
						</a>
					</td>
					<td>
						<a href="admin_delete_gallery_video.php?id='.$row['video_id'].'&pid='.$id.'" style="padding-left: 10px;" class="delete btn btn-danger btn-sm btn-icon icon-left">
							Delete
						</a>
					</td>
				</tr>
