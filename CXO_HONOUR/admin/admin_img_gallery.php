...
<h2>Image Gallery</h2>
<div style="float:right"><a href="admin_add_img_gallery.php"><button type="button" class="btn btn-success">Add New</button></a></div>
<br />
<br />

<?php if(isset($_REQUEST['del'])){
echo '<h2 style="color:red;">Image Gallery Deleted</h2>';
}
if(isset($_REQUEST['edit'])){ 
echo '<h2 style="color:green;">Image Gallery Updated</h2>';
}
if(isset($_REQUEST['add'])){
echo '<h2 style="color:green;">Image Gallery Added</h2>'; 
}
?>
		...
		{ 
			...
			echo'
				...
				$result2 = mysql_query("SELECT * FROM `gallery_stuff` WHERE `gallery_id` = '$gallery_id '")or die (mysql_error());
					...
					{ 
						echo '&nbsp;<img src="upload/gallery/images/'.$row2['stuff'].'" width=80px/>';
					}
					
			echo '</td><td>'.$row['date'].'</td>
					<td><a href="admin_edit_img_gallery.php?id='.$row['gallery_id'].'" class="btn btn-default btn-sm btn-icon icon-left edit_tour_list">
							<i class="entypo-pencil"></i>
							Edit
						</a>
					</td>
					<td>
						<a href="admin_delete_img_gallery.php?id='.$row['gallery_id'].'" style="padding-left: 10px;" class="delete btn btn-danger btn-sm btn-icon icon-left" onclick="return confirm(\'Are you sure?\');">
							Delete
						</a>
					</td>
				</tr>
