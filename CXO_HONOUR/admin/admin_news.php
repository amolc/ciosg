...
<h2>News</h2>
<div style="float:right"><a href="admin_add_news.php"><button type="button" class="btn btn-success">Add New</button></a></div>
<br />
<br />

<?php if(isset($_REQUEST['del'])){
echo '<h2 style="color:red;">News Deleted</h2>';
}
if(isset($_REQUEST['add'])){
echo '<h2 style="color:green;">News Added</h2>';
}
if(isset($_REQUEST['edit'])){
echo '<h2 style="color:green;">News Updated</h2>'; 
}
...
$result = mysql_query("SELECT
		...
		{ 
			...
			echo'
				<tr class="odd gradeX" id="'.$row['news_id'].'">				    
					<td><a target="_blank" href="../view_news.php?id='.$row['news_id'].'">'.$row['news_title'].'</a></td>					
					<td><img src="upload/news/'.$row['news_img'].'" width=80px/></td>
					<td>'.$row['news_inserted_date'].'</td>
					<td><a href="admin_edit_news.php?id='.$row['news_id'].'" class="btn btn-default btn-sm btn-icon icon-left edit_tour_list">
							<i class="entypo-pencil"></i>
							Edit
						</a>
					</td>
					<td>
						<a href="admin_delete_news.php?id='.$row['news_id'].'" style="padding-left: 10px;" class="delete btn btn-danger btn-sm btn-icon icon-left" onclick="return confirm(\'Are you sure?\');">
							Delete
						</a>
					</td>
				</tr>
