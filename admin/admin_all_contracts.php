...
echo '<h2 style="color:red">Contracts Deleted</h2>'; 
}
if(isset($_REQUEST['edit'])){
echo '<h2 style="color:green">Contracts Updated</h2>'; 
}if(isset($_REQUEST['add'])){
echo '<h2 style="color:green">Contracts Added</h2>'; 
}
 ?>
<br />
...
$result = mysql_query("SELECT
		...
		{ 
			$query = mysql_query("SELECT * FROM	all_users where registration_id =".$row['vID']);
			while ($rs = mysql_fetch_array($query)) 
			{
				$vendor_name = $rs['firstname']." ".$rs['lastname'];
			}
			echo'
				<tr class="odd gradeX" id="'.$row['contract_id'].'">
				
					<td>'.$row['contract_id'].'</td>
					<td>'.$vendor_name.'</td>					
					<td>'.$row['company_name'].'</td>
					<td>'.$row['date'].'</td>
					<td><a href="admin_edit_contract.php?id='.$row['contract_id'].'" class="btn btn-default btn-sm btn-icon icon-left edit_tour_list">
							<i class="entypo-pencil"></i>
							Edit
						</a>
					</td>
					<td>
						<a href="admin_contract_delete.php?id='.$row['contract_id'].'" style="padding-left: 10px;" class="delete btn btn-danger btn-sm btn-icon icon-left" onclick="return confirm(\'Are you sure?\');">
							Delete
						</a>
					</td>
				</tr>
