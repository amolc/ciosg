...
include('../sql_config/database/cio_db.php'); 
 $id = $_REQUEST['id'];
 $res = mysql_query("select * from ict_vendor_landing  where id = '$id'")or die (mysql_error());
...
<?php include('include/admin_header/admin_header.php'); ?>
			...
			<ol class="breadcrumb bc-3">
				...
				<li class="active">
					<strong>ICT Vendor Landing Slider</strong>
				</li>
			</ol>

			<h2>ICT Vendor Landing Slider</h2>
			<br />
...
<div class="row">
	<div class="col-md-12">
		...
		<div class="panel panel-primary" data-collapsed="0">
			...
			<div class="panel-heading">
				<div class="panel-title">
					ICT Vendor Landing Slider Edit
				</div>
		...
		<?php
 				ini_set('upload_tmp_dir', '/landing_images');
				 include('../../sql_config/database/cio_db.php'); 
...
//					echo "Temp file: " . $_FILES["file"]["tmp_name"] . "<br />";
					...
					if (file_exists("landing_images/ict_vendor/" . $_FILES["file"]["name"]))
					  {
					  	 $fileData = pathinfo(basename($_FILES["file"]["name"]));
					   	 $fileName = uniqid() . '.' .$fileData['extension'];
						 $target_path = 'landing_images/ict_vendor/'.$fileName;
					     move_uploaded_file($_FILES["file"]["tmp_name"],$target_path);
					    // echo "Stored in: " . "admin/" . $target_path;
					  }
					else
					  {
				       $fileData = pathinfo(basename($_FILES["file"]["name"]));
				...
				else
				  {
					echo "Invalid file";
						...
						$result = mysql_query("SELECT id FROM ict_vendor_landing WHERE id = '$id'");
						...
						if ($num_rows > 0)
						{
							//echo "exist";
                                                    if(!empty($path)){
							$sql   = "UPDATE ict_vendor_landing SET title='".$title."',description='".$description."',insert_date='".$insert_date."',path='".$path."' WHERE id = '$id'";
                                                    }else
                                                    {
                                                        $sql   = "UPDATE ict_vendor_landing SET title='".$title."',description='".$description."',insert_date='".$insert_date."' WHERE id = '$id'";
                                                    }
						}
						else
						{
							//echo "does not exist";
							$sql   = "insert into ict_vendor_landing(title,description,insert_date,path) values ('$title','$description','$insert_date','$path')";       
				...
				<form role="form" action="<?php $_SERVER["PHP_SELF"];?>" method="post"  enctype="multipart/form-data" class="form-horizontal form-groups-bordered">
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input type="text" class="form-control" id="field-1" name="title" placeholder="Title" value="<?php echo $row['title']; ?>">
						</div>
					</div>
					   <div class="form-group">
                                        ...
                                        <div class="col-sm-5">
                                            <input type="text" class="form-control" id="field-1" name="description" placeholder="Link " value="<?php echo $row['description']; ?>">
                                        </div>
                                    </div>
							<div class="form-group">
