...
class Zll extends CI_Model {
	...
	function addToEmailNotification($wID,$wuID,$rTT,$rTime,$wrs,$un,$ue,$web_link)
		{
			$curr_time = time();
	...
	function get_site_url($https)
		{
			...
			if($_SERVER['SERVER_NAME']=='www.globalprompt.org')
			...
			else if($_SERVER['SERVER_NAME']!='192.168.1.4' && $_SERVER['SERVER_NAME']!='localhost' )
				{
			...
			else
				{
					if($_SERVER['SERVER_NAME']=='localhost')
						...
						$ret_url	= "http://192.168.1.4/dropbox/newic/";
				}
			return $ret_url;
		}
...
//			curl_close ($ch);
			...
			if(!$lang) $lang = 'spa';
			$this->load->library('email');			
			$template_data = file_get_contents(getcwd().'/email_temp.html');
			$gp[0] = '/{DATE}/';
			$gp[1] = '/{TEMPLATE_DATA}/';
			$gp[2] = '/{UNSUBSCRIBE_LINK}/';
...
$q = $this->db->update('messages', $data); 
 ...
 $this->db->delete('messages'); 
  ...
  return true;
	...
	function adminlog($ld,$status)
		{
		
			         $this->load->library('user_agent');
			         $log_ins['logdata'] = $ld;
		             $log_ins['asID'] = $this->session->userdata('asID');
			         $log_ins['username'] = $this->session->userdata('admin_username');
			         $log_ins['ipAddress'] = $_SERVER['REMOTE_ADDR'];
			         $log_ins['browser'] = $this->agent->browser().' '.$this->agent->version();
			         $log_ins['date_time'] = date("Y:m:d H:i:s");
			         $log_ins['status'] = $status;
			         $log = $this->zll->gInsert('adminlog',$log_ins);
		}
...
function Sec2Time($time){
  if(is_numeric($time)){
		...
		function make_bitly_url($url,$format = 'xml',$version = '2.0.1')
	{
		...
		$login="nikhilmusale91";
		$appkey="R_c134563be1b24d2e882475ac1585185e";	
