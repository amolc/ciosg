...
function add_item()
	{
		...
		if(item_name==''||category==''||item_price=='')
		{
			error+='Item name and category required';
		}
		else
		{
				document.getElementById('add_item_result').innerHTML =  "Saving...";  
				$('#add_item_result').show().delay(5000).hide(0);
				$.getJSON('http://www.globalprompt.org/sg/cio/category/add_item/?callback=?',"item_name="+item_name+"&item_desc="+item_desc+"&category="+category+"&item_price="+item_price+"",function(res){
...
function show_category(h)
{
	...
	$.getJSON('http://www.globalprompt.org/sg/cio/category/list_category/?callback=?' , function(array) {
		...
		var i=0;	
		var catID='';
		optn += "<option value=''>Select</option></td>";
		 $.each(array, function(key,val) {
...
function set_item_data(itemid)
 {
 	...
 	$.getJSON('http://www.globalprompt.org/sg/cio/category/get_item/?callback=?' ,"itemid="+itemid+"", function(array) {
		...
		jQuery('#edit_item_price').val(array.item_data.item_price);
 ...
 {     
		...
		var edit_item_price=document.getElementById('edit_item_price').value;
		var edit_category=document.getElementById('edit_category').value;
		
		if(edit_item_name==''|| edit_category==''||edit_item_price=='')
		{
			error+='item name,price and category required';
		}
		else
		{	
			document.getElementById('edit_item_result').innerHTML = "Updating...";
			$('#edit_item_result').show().delay(5000).hide(0);
			$.getJSON('http://www.globalprompt.org/sg/cio/category/edit_item/?callback=?',"edit_item_ID="+edit_item_ID+"&edit_item_name="+edit_item_name+"&edit_item_desc="+edit_item_desc+"&edit_category="+edit_category+"$edit_item_price="+edit_item_price+"",function(res){
...
function accept_cio(cid)
{
	...
	$.getJSON('http://www.globalprompt.org/sg/cio/category/accept_cio/?callback=?' ,"cid="+cid+"",function(array) {
				...
				var firstname=array.email_data.firstname;
				var emailID=array.email_data.emailID;
				var registration_type=array.email_data.registration_type;
				var password=array.email_data.password;
				document.getElementById('pclist').innerHTML="Working..."; 
				$.post("activation_link2.php" ,{'name':firstname ,'email':emailID,'reg_type':registration_type,'pass':password}).done(function( data ) {   
							...
							if(data=="OK")
							{
								$.getJSON('http://www.globalprompt.org/sg/cio/category/cio_accepted/?callback=?' ,"cid="+cid+"",function(array) {
									 list_pending_cio();
									 document.getElementById('pclist').innerHTML=array.result;   
  									 $('#pclist').show().delay(5000).hide(0);
									 });
							}
...
function accept_vendor(vid)
{
		...
		$.getJSON('http://www.globalprompt.org/sg/cio/category/accept_vendor/?callback=?' ,"vid="+vid+"",function(array) {
				$.post("activation_link2.php" ,{'name':array.email_data.firstname ,'email':array.email_data.emailID,'reg_type':array.email_data.registration_type,'pass':array.email_data.password}).done(function( data ) {   
