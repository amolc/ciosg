...
/*function listCategory()
{
    ...
    $.each(array, function(key,val) {
		...
		tbl_row +="<div class='clsLD_cont2 clearfix'><div class='clsChart_bx' style='text-align:center;'><div id='donutchart_"+i+"' ></div></div></div>";
...
function get_div(id)
	...
	/* function pie_chart(pid)
	{
      google.load("visualization", "1", {packages:["corechart"]});
      google.setOnLoadCallback(drawChart);
      function drawChart() {
	  var i=0;
	  	$.getJSON('piechart_data.php?callback=?' , function(array) {
				$.each(array, function(key,val) {
				var tbl_row = "";
				$.each(this, function(k , v) {
				var item_name=array[i].item_name;
		...
		for (var i=0;i<9;i++)
		{
       	 	var chart = new google.visualization.PieChart(document.getElementById('donutchart_'+pid));
        	chart.draw(data, options);
		}
      }
	 }*/
...
<div id="black_wrapper">
    <div class="inner_nav">
		<div class="nav fl">
        	<ul>
			  <li><a class="menu_ancher" href="javascript:void(0);" onClick="get_div(9);">HOME</a></li>
...
<div class="landing_head advisory_wrapper" style="margin-top:0px;" id="div_8" >
     <div class="clsMiddle">
		<div class="clsMid_cont_cio" id="category_div">
			...
			{ 
				...
				?>
				<input type="hidden" id="cid" value="<?php echo $n;?>">
    		<script type="text/javascript">
			var n=document.getElementById('cid').value;
      			...
      			function drawChart() {
					...
					<?php $result=mysql_query("select item.item_name,survey_count.count from survey_count,item where categoryID='1' and item.item_ID=survey_count.itemID order by count desc limit 3 ") or die(mysql_error());	
					while($rs=mysql_fetch_array($result)){?>
					 
					
					  ['<?php echo $rs['item_name']; ?>', '<?php echo $rs['count'];?>'],
		...
		<?php for($i=0;$i<$n;$i++)
		{ ?>
      		<div id="donutchart_"<?php echo $i; ?> style="width: 900px; height: 500px;"></div>
		<?php } ?>
...
<div class="landing_head advisory_wrapper" style="margin-top:0px;" id="div_9" >
     <div class="clsMiddle">
		<div class="clsMid_cont_cio" id="category_div">
			<div id="container1"></div>
				...
				?>
					...
					$num_results = mysql_num_rows($result);
						...
						while( $row = mysql_fetch_array($result) ){
							...
							$dataSet->addPoint(new Point("{".$row['item_name']."} {".$row['count']."})", $row['count']));
						}
						 //finalize dataset
						...
						echo "<img alt='Pie chart'  src='images/1.png' style='border: 1px solid gray;'/>";
